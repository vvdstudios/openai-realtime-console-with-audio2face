...
export function ConsolePage() {
  ...
  /**
   * Establish WebSocket connection and initialize WavStreamPlayer
   */
  useEffect(() => {
    ...
    return () => {
      if (isUnmounting.current) {
        // Clean up only if the component is unmounting
  ...
  useEffect(() => {
    return () => {
      // Set the unmounting flag to true
   ...
   * WavRecorder takes speech input, WavStreamPlayer handles output, client is API client
   */
  ...
  const startRecording = async () => {
    ...
    const wavStreamPlayer = wavStreamPlayerRef.current;
    if (wavStreamPlayer) {
      await wavStreamPlayer.interrupt(); // Send interruption signal to server
    }
    await wavRecorder.record((data) => client.appendInputAudio(data.mono));
  };
  ...
  useEffect(() => {
    ...
    client.on('conversation.updated', async ({ item, delta }: any) => {
      ...
      const wavStreamPlayer = wavStreamPlayerRef.current;
      if (delta?.audio && wavStreamPlayer) {
